<template>
  <div>
    <div class="relative">
      ...
      <label
        :for="id"
        class="absolute text-sm text-light-text/60 dark:text-dark-text/60 duration-300 transform -translate-y-4 scale-75 top-[5px] z-10 origin-[0] bg-light-background dark:bg-dark-background px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-[5px] peer-focus:scale-75 peer-focus:-translate-y-4 peer-focus:text-light-accent dark:peer-focus:text-dark-accent left-3"
      >
        {{ label }}
      </label>
    </div>
    <div v-if="error" :id="`${id}-error`" class="text-red-500 text-xs mt-2">
      {{ error }}
    </div>
  </div>
</template>
